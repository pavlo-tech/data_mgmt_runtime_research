...
fast_df['FLOPS']= (2 * fast_df['matrix_width']**3) / (fast_df['Time'])
fast_df['Megaflops']=fast_df['FLOPS']/2**20
#print(tiled_df)
...
for cache_size in [0]:
	...
	for iw, width in enumerate(widths):
		...
		ucBar_pos=iw*(1+ 3*len(rates)+ 1.5)*bar_width
		print(ucBar_pos)
		...
		for ir, rate in enumerate(rates):
			fast_mflops=fast_df.loc[(fast_df['is_zfp'] == True) & (fast_df['data_type'] == 'double') &(fast_df['is_fast'] == True)&\
				(fast_df['cache_size'] == cache_size) & (fast_df['matrix_width'] == width) & (fast_df['zfp_rate'] == rate)]['Megaflops'].mean()
			xpos=ucBar_pos +(2*len(rates) +ir+1)*bar_width
			print(xpos)
			plt.bar(xpos, fast_mflops, bar_width, color=colors[ir],hatch='+')

		xticks.append((xpos + ucBar_pos)/2)
	plt.xticks(xticks, widths)
	x=plt.bar(0,0,0,color='w',edgecolor='k',hatch='//', label='Tiled')
	y=plt.bar(0,0,0,color='w',edgecolor='k',hatch='+', label='Fast')
	legend = [mpl.patches.Patch(color='k', label='C Array'), x, y]
	...
	plt.xlabel('n')
	plt.tight_layout()
	plt.savefig("images/barchart_fast_"+str(cache_size)+".pdf")			

...
int readMM(const char filename[], csr_matrix* A) {
    ...
    A->V = zfp::array1<double>(A->nnz,RATE,0,CSIZE);//(double *) malloc(A->nnz * sizeof(double));
 ...
 */
void swapD(zfp::array1<double>::pointer a, zfp::array1<double>::pointer b) {
    zfp::array1<double>::pointer t = a;
    a = b;
    b = t;
}
